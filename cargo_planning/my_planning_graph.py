...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        1. determine what actions are available in self.all_actions 
        2. Add the available actions to  PgNode_a objects and I will name it as action_node
        """
        action_nodes =[]
        for action in self.all_actions:
            ...
            """
            I can reach this action_node only when the conditions in the previous STATE 
            at level s_levels has been met
            
            This same as the above mentioned GIVEN comment: 
            ...
            if  action_node.prenodes.issubset(self.s_levels[level]):
                ...
                for state_node in (self.s_levels[level]):
                    """
                    ACTION will get added to CHILD of STATE NODE
                    STATE will get added to PARENT of ACTION NODE
                    """
        ...
        GIVEN
        :return:
            adds A (ACTION) nodes to the current level in self.a_levels[level]
        """            
    ...
    def add_literal_level(self, level):
        ...
        add literal S level to the planning graph
        ...
        1. SO GET THE ACTION NODE FROM THE PREVIOUS LEVEL 
        2. FIND THE STATE NODE FROM THE PREVIOUS EFFECT NODE
        """
        for action_node in self.a_levels[level-1]:
            for effect_node in action_node.effnodes:
                ...
                """
                state_nodes.add(effect_node) # Adding Effect Nodes to the set of state node
                effect_node.parents.add(action_node) # Preceding Action Node is the Parent of Effect Node
                action_node.children.add(effect_node) # Effect Node is the Children of Action Node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        GIVEN :
            Test a pair of actions for inconsistent effects, returning True if
            one action negates an effect of the other, and False otherwise.
        MEANS THERE ARE 2 ACTION NODE:
            node_a1.action and node_a2.action
        AND THEREFORE TWO EFFECTS FOR EACH ACTION :
            node_a1.action.effect_add node_a1.action.effect_rem  AND node_a2.action.effect_add node_a2.action.effect_rem
            
        SO I WILL
        
        1. Find all the effects of ACTION  node_a1  where the effect is ADDING effect_add
            Means for effects in node_a1.action.effect_add
        2. Check if the above effets of ACTION  node_a1 fall in the list of node_a2 where the effect is REMOVING effect_rem
           Means effects of node_a1.action.effect_add NOT IN  node_a2.action.effect_rem --- IF THIS IS TRUE THEN RETURN
           FALSE --> Means No Inconsistency
           
        NOW REVERSE and Check for Inconsistency OTHER WAY ROUND
        3. Find all the effects of ACTION  node_a2  where the effect is ADDING effect_add
            Means for effects in node_a2.action.effect_add
            
        4.  Check if the above effets of ACTION  node_a2 fall in the list of node_a1 where the effect is REMOVING effect_rem
           Means effects of node_a2.action.effect_add NOT IN  node_a1.action.effect_rem --- IF THIS IS TRUE THEN RETURN
           FALSE --> Means No Inconsistency
        
        """
        for effects_of_action1 in node_a1.action.effect_add:
            if effects_of_action1  in node_a2.action.effect_rem:
                return True
            
        for effects_of_action2 in node_a2.action.effect_add:
            if effects_of_action2  in node_a1.action.effect_rem:
                return True    
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        AND THEREFORE TWO PRE CONDITION FOR EACH ACTION :
        node_a1.action.precond_pos, node_a1.action.precond_neg, node_a2.action.precond_pos, node_a2.action.precond_neg
        
        SO I WILL HAVE TO CHECK THE FOLLOWING:
            1. For all effects of node_a1.action.effect_add if any effects falls in the list of node_a2.action.precond_neg
                Then return TRUE --> Means Interference, you are trying to add where precondition is negative
                
            2. For all effects of node_a1.action.effect_rem if any effects falls in the list of node_a2.action.precond_pos
                Then return TRUE --> Means Interference, you are trying to remove where precondition is positive
            
            3. For all effects of node_a2.action.effect_add if any effects falls in the list of node_a1.action.precond_neg
                Then return TRUE --> Means Interference, you are trying to add where precondition is negative
                
            4. For all effects of node_a2.action.effect_rem if any effects falls in the list of node_a2.action.precond_pos
                Then return TRUE --> Means Interference, you are trying to remove where precondition is positive    
        ...
        for  effects_action1_add in node_a1.action.effect_add:
            if effects_action1_add in node_a2.action.precond_neg:
                return True
       
        """
        Condition 2.
        """
        for  effects_action1_rem in node_a1.action.effect_rem: 
           if effects_action1_rem in node_a2.action.precond_pos:
                return True
        """
        Condition 3.
        """
        for  effects_action2_add in node_a2.action.effect_add:
            if effects_action2_add in node_a1.action.precond_neg:
                return True
        """
        Condition 4.
        """    
        for  effects_action2_rem in node_a2.action.effect_rem: 
           if effects_action2_rem in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        HOW DO I FIND PRECOND:
        In the method   def precond_s_nodes(self) Following is mentioned 
        precondition literals as S-nodes (represents possible parents for this node).
        
        MEANS : I can looks for a PARENT for the ACTION and COMPARE IT in is_mutex()
       
        """
        
        for precond_of_action1 in node_a1.parents:
            for precond_of_action2 in node_a2.parents:
                """
                Return True if precond_of_action1 and precond_of_action2 is MUTEX
                """
                return precond_of_action1.is_mutex(precond_of_action2) 
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        FROM ABOVE HINT :
            1. defines the notion of equivalence for   literal expression nodes
            2. class tracks whether the literal is         positive or negative 
            
        SO Here the expression is Symbol , therefore I will check equivalency of node_s1.symbol==node_s2.symbol
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for precond_of_state1 in node_s1.parents:
            for precond_of_state2 in node_s2.parents:
                """
                Return True if precond_of_action1 and precond_of_action2 is MUTEX
                """
                if not precond_of_state1.is_mutex(precond_of_state2) :
            ...
            while i < length:
                for s in self.s_levels[i]:
                    if goal == s.symbol:
                        ...
                        print( " level sum After =" ,level_sum + i," level sum Before =",level_sum, "i=",i, "goal=" , goal," s.symbol=",s.symbol, "length =", length)
                        level_sum = level_sum + i
                        i = length

                    else:
                        print("Else :   level sum After = ", level_sum + i," level sum  Before=",level_sum, "i=",i, "goal=" , goal," s.symbol=",s.symbol, "length =", length)
                        i = i +1   
        return level_sum
        """
#######################  WORKED ###############################3
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    if goal == state.symbol and state.is_pos:
                        level_sum += level
#                        print("CURRENT LEVEL SUM =",level_sum,"SYMBOL =", state.symbol," SYMBOL BY EXP =",expr(state.symbol), " GOAL =",goal," IS POS =",state.is_pos,"LEVEL =",level,"\n", "DICT =",state.__dict__ ,"\n","PARENTS =",state.parents, "\n" )
                        goal_found = True
                if goal_found:
                    break
            goal_found = False
